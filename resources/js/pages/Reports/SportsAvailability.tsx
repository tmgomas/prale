...
const breadcrumbs = [
    ...
    { title: 'Reports', href: '#' },
    { title: 'Sports Availability Matrix', href: '#' },
];

export default function SportsAvailability({ districts, sports, selectedDistrictId, matrixData }: Props) {
    const handleDistrictChange = (value: string) => {
        router.get('/reports/sports-availability', { district_id: value }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    const handleExport = () => {
        window.location.href = '/reports/sports-availability/export';
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Sports Availability" />
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
                            Sports Availability Framework
                        </h1>
                        <p className="text-muted-foreground">
                            Check sport availability across divisions without quantities.
                        </p>
                    </div>
                ...
                {!selectedDistrictId ? (
                    <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed border-2 bg-muted/30">
                        ...
                        <CardDescription className="max-w-md">
                            Please select a district from the dropdown above to view the divisional sport availability matrix.
                        </CardDescription>
                    </Card>
                ) : (
                    <div id="printable-area">
                        <div className="hidden print:block mb-6">
                            <h1 className="text-2xl font-bold">Sports Availability Matrix</h1>
                            <p className="text-muted-foreground">
                            ...
                            <CardHeader className="border-b bg-muted/50 py-4 print:bg-transparent print:p-0 print:border-none print:mb-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <CardTitle className="text-lg">Divisional Availability Status</CardTitle>
                                    </div>
                                    ...
                                    <table className="w-full border-collapse">
                                        <thead>
                                            <tr className="bg-muted/30 border-b print:bg-gray-100" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                                                <th rowSpan={2} className="sticky left-0 z-20 bg-background border-r p-4 text-left font-bold text-sm min-w-[150px] print:static print:bg-transparent print:p-1 print:min-w-[50px] print:text-xs">
                                                    Division Name
                                                </th>
                                                {sports.map((sport) => (
                                                    <th
                                                        key={sport.id}
                                                        colSpan={2}
                                                        className="p-3 text-center border-r font-medium text-xs min-w-[70px] hover:bg-muted/50 transition-colors print:min-w-0 print:p-1"
                                                    >
                                                        <TooltipProvider>
                                                            <Tooltip>
                                                                <TooltipTrigger asChild>
                                                                    <div className="flex flex-col items-center gap-1 cursor-help print:cursor-default">
                                                                        ...
                                                                        <span className="truncate max-w-[60px] print:max-w-none print:text-[10px]">{sport.name_en}</span>
                                                                    </div>
                                                                </TooltipTrigger>
                                                                <TooltipContent>
                                            ...
                                            <tr className="bg-muted/30 border-b print:bg-gray-100" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                                                {sports.map((sport) => (
                                                    <React.Fragment key={sport.id}>
                                                        <th className="p-1 text-center border-r text-[10px] font-semibold text-blue-700 bg-blue-50/20 print:text-[8px] print:p-0 min-w-[35px]">
                                                            Men
                                                        </th>
                                                        <th className="p-1 text-center border-r text-[10px] font-semibold text-pink-700 bg-pink-50/20 print:text-[8px] print:p-0 min-w-[35px]">
                                                            Women
                                                        </th>
                                                    </React.Fragment>
                                                ))}
                                                    ...
                                                    {normalDivisions.map((division) => (
                                                        <tr key={division} className="border-b transition-colors hover:bg-primary/5 group print:break-inside-avoid">
                                                            ...
                                                            {sports.map((sport) => {
                                                                ...
                                                                const hasMen = data && (data.men_teams > 0 || data.men_participants > 0);
                                                                const hasWomen = data && (data.women_teams > 0 || data.women_participants > 0);

                                                                return (
                                                                    <React.Fragment key={sport.id}>
                                                                        {/* Men Availability */}
                                                                        <td className="p-0 border-r text-center bg-blue-50/5">
                                                                            <div className="flex items-center justify-center h-10 w-full print:h-6">
                                                                                {hasMen ? (
                                                                                    <Check className="w-5 h-5 text-blue-600 print:text-black print:w-3 print:h-3" />
                                                                                ) : (
                                                                        ...
                                                                        <td className="p-0 border-r text-center bg-pink-50/5">
                                                                            <div className="flex items-center justify-center h-10 w-full print:h-6">
                                                                                {hasWomen ? (
                                                                                    <Check className="w-5 h-5 text-pink-600 print:text-black print:w-3 print:h-3" />
                                                                                ) : (
                                                                                    <span className="text-muted-foreground/30 print:hidden">-</span>
                                                                                )}
                                                                            </div>
